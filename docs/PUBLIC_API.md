...
| Command | Purpose |
| --- | --- |
| `pci index` | Main indexing command. Requires one or more repository paths, such as `pci index .`. Can emit MCP client setup and required environment exports with `--mcp-config {env,codex,claude,opencode,vscode,copilot,cline,zed,pi}`. Low-level ingest flags are reachable via `pci index ... -- <flags>`. |
