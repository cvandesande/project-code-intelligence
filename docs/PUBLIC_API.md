...
| `list_code_intel_files` | List indexed source files filtered by language, role, content class, or skip status. Includes record-backed paths when searchable records exist without a file inventory row. Useful for discovering the shape of the codebase. |
