...
| `pci audit --full-triage --init-triage` | Creates or extends `.pci/audit-triage.json` with language-agnostic, collection/repository-scoped redundancy candidate IDs. Use `--candidate ID --status open` or `--candidate ID --status dismissed --reason TEXT` to record a disposition. A unique overlap of at least two members preserves the disposition when a group expands or contracts; ambiguous splits/merges remain new candidates. Saved candidates absent from a later `--full-triage` sweep are reported as fixed, while bounded reports never infer that omitted candidates are fixed. Writes are locked and atomic; the shared state records timestamps and the generating PCI version. Candidate discovery still depends on PCI's parser and call-edge coverage for each language. `--triage-file PATH` selects another state file. |
