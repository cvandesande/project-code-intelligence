...
from project_code_intelligence import db, llama_embed, process
...
def llama_batch_embeddings(texts: list[str], batch_size: int) -> list[str]:
    ...
    for offset in range(0, len(texts), batch_size):
        ...
        with tempfile.NamedTemporaryFile("w", encoding="utf-8", suffix=".txt") as prompt:
            ...
            _ = prompt.flush()
            command = llama_embed.build_command(Path(prompt.name))
            command.extend(["--embd-separator", separator.strip()])
            env = os.environ.copy()
            lib_path = str(llama_embed.llama_dir())
            env["LD_LIBRARY_PATH"] = (
            ...
            try:
                proc = process.run(
                    ...
                    process.RunOptions(
                        ...
                        capture_output=True,
                        env=env,
                        timeout=llama_embed.llama_timeout_seconds(),
                    ),
                )
        ...
        parsed = llama_embed.parse_llama_stdout(proc.stdout)
        ...
        vectors.extend(db.vector_literal(llama_embed.extract_embedding(item)) for item in data)
    return vectors
