...
GENERATED_HEADER_RE = re.compile(r"(?im)^(?://|#|/\*)\s*Code generated\b.*\bDO NOT EDIT\.?")
GO_CODEGEN_CLIENT_PARTS = frozenset({"applyconfiguration", "clientset", "informers", "listers"})
...
def has_generated_header(text: str | None) -> bool:
    if not text:
        return False
    return bool(GENERATED_HEADER_RE.search(text[:4096]))


def generated_path_hint(path: str, language: str) -> bool:
    parts = path.split("/")
    lower_parts = tuple(part.lower() for part in parts)
    file_path = Path(path)
    name = file_path.name.lower()
    if any(part in {"generated", "autogenerated"} for part in lower_parts) or "generated" in name:
        return True
    if name.startswith("zz_generated.") or name.endswith("_generated.go"):
        return True
    if language == "go" and any(part in GO_CODEGEN_CLIENT_PARTS for part in lower_parts):
        return "client" in lower_parts or "pkg" in lower_parts
    return False


def classification_flags(path: str, language: str, text: str | None = None) -> dict[str, bool]:
    parts = path.split("/")
    file_path = Path(path)
    name = file_path.name.lower()
    suffix = file_path.suffix.lower()
    is_test = any(part in {"test", "tests", "testing", "selftests"} for part in parts) or "test" in name
    is_generated = generated_path_hint(path, language) or has_generated_header(text)
...
def classify_file(path: str, language: str, text: str | None = None) -> JsonObject:
    flags = classification_flags(path, language, text)
...
def discover_files(
    ...
    *,
    ...
    for git_blob_sha, rel_path in sorted(git_ls_files(repo_root), key=itemgetter(1)):
        ...
        classified = profile_context.active_profile.classify_file(
            rel_path,
            language,
            classify_file(rel_path, language, text),
        )
